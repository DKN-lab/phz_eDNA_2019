...
plot_toto <- ggplot(wt_pred_summary %>% filter(Condition == 'TOTO'), aes(x = Colony_number, y = mean_eDNA)) + 
  ...
  geom_jitter(data = wt_preds %>% filter(Condition == 'TOTO'), aes(y = eDNA_dsDNA_uM), shape = 21, width = 0.1, height = 0)+
  geom_pointrange(aes(ymin = mean_eDNA - sd_eDNA, ymax = mean_eDNA + sd_eDNA)) + 
  ylim(0,NA) + 
...
```{r}
sub_panel <- plot_grid(plot_redox + labs(title = NULL), plot_toto,dnase_plot_styled, ncol = 3, rel_widths = c(0.5, 0.5, 1), align = 'hv', axis = 'tblr', labels = c('B','C'))

sub_panel
```

```{r}
bottom_panel <- plot_grid(plot_pel_styled, plot_dphz_etbr,
                          ncol = 2, align = 'hv', axis = 'tblr')

plot_grid(sub_panel, bottom_panel, ncol = 1, align = 'hv', axis = 'tblr')

right_panel <- plot_grid( sub_panel, dnase_plot_styled, 
                          plot_pel_styled, plot_dphz_etbr,
                          ncol = 2, align = 'hv', axis = 'tblr', labels = c('','D','E','F'))

right_panel

plot_grid(NULL, sub_panel, dnase_plot_styled, 
          NULL, plot_pel_styled, plot_dphz_etbr,
          ncol = 3, align = 'hv', axis = 'tblr', labels = c('A','','D','','E','F'))

library(patchwork)

((plot_redox | plot_toto | dnase_plot_styled ) + plot_layout(widths = c(0.5,0.5,1)) ) / (plot_pel_styled | plot_dphz_etbr)
```
